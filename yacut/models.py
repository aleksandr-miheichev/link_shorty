...
class URLMap(db.Model):
    ...
    original = db.Column(db.String(MAX_LINK_LENGTH), nullable=False, index=True)
    ...
    def to_dict(self):
        ...
        return dict(
            url=self.original,
            short_link=url_for(FUNCTION_REDIRECT, custom_id=self.short, _external=True),
        )
        ...
        if validate:
            ...
            if custom_id:
                ...
                if not URLMap.is_custom_id_unique(custom_id):
                    raise InvalidORMUsage(ID_AVAILABLE_API.format(custom_id=custom_id))
        if not custom_id:
